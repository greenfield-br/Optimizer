...
def make_header(output, strategy, timeframe, period_ini, period_end, min_buycount, max_avg_duration, fee):
	...
	with open(output +_suffix +'.csv','w') as f1:
		...
		writer.writerows([['Strategy,', strategy], ['Timeframe,', timeframe], ['Period,', period_ini+'-'+period_end], ['Startup candle,', 30], ['Fees [%],', fee * 100], ['Minimum buy count requisit,', min_buycount], ['Maximum avg. duration requisit [h],', max_avg_duration], [], ['window buy,', 'window sell,', 'standard dev.,', 'stop loss,', 'trailing,', 'roi,', 'trailing pos. stop loss offset,', 'pos. stop loss,',  'buy count,', 'avg. profit [%],', 'cum profit [%],', 'total profit [USDT],', 'total profit [%],', 'avg. duration,', 'avg. duration [h],', 'profit,', 'loss,', 'force sell,', 'sell signal,', 'stop loss,', 'roi,', 'trailing stop loss,']])


def backtesting_and_save(_pair, _timeframe, inf_pair, inf_timeframe, _period_ini, _period_end, _bb_buy, _bb_sell, _std, _stoploss, _trailing_stop_bool, _roi, _trailing_stop_pos_offset, _trailing_stop_pos, strategy, fee, path, output):
	# create paths
	...
	with open(path_base_strategy,'r') as f1:
		...
		data = data.replace('bb_value_buy', str(_bb_buy))
		data = data.replace('bb_value_sell', str(_bb_sell))
		data = data.replace('std_value', str(_std))
	...
	with open(path +'/freqtrade/config_default.json','r') as f1:
		data = f1.read()
...
#	with open(output +_suffix +'.txt','a') as f2:
		parameters = [_bb_buy, _bb_sell, _std, _stoploss, _trailing_stop_bool, _roi, _trailing_stop_pos_offset, _trailing_stop_pos]
...
def best_value(best_bb_value_buy, best_bb_value_sell, best_std_value, best_stoploss, best_trailing_stop_bool, best_roi, best_trailing_stop_pos_offset, bb_value_buy, bb_value_sell, std_value, stoploss, trailing_stop_bool, roi, trailing_stop_pos_offset, results, best_result, max_avg_duration, min_buycount):
	profit_per = float(results[12])
	buycount = int(results[8])
	avg_duration = int(results[14])

	if (profit_per > best_result and avg_duration < max_avg_duration and buycount > min_buycount):
		best_bb_value_buy  = bb_value_buy
		best_bb_value_sell = bb_value_sell
	...
	return best_bb_value_buy, best_bb_value_sell, best_std_value, best_stoploss, best_trailing_stop_bool, best_roi, best_trailing_stop_pos_offset, best_result
