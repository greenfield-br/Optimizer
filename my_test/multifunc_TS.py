...
def dict_exFile3(home, path, k, _k, filename):
	...
	dataframe = pandas.read_csv(_filename_opt, sep=',', skiprows=8, nrows=450)
	...
	pivot = dataframe.groupby(by=['timeperiod buy', 'timeperiod sell']).median()
	narray_exPivot = pivot['cum profit [%]'].to_numpy().reshape(21,21)
	narray_exPivot2 = pivot['buy count'].to_numpy().reshape(21,21)
	x_plot = numpy.unique(dataframe['timeperiod buy'].tolist())
	y_plot = numpy.unique(dataframe['timeperiod sell'].tolist())
	x_plot3 = pivot['cum profit [%]'].tolist()
	y_plot3 = pivot['buy count'].tolist()
	_dict = {
		'_narray_exPivot': narray_exPivot,
		'_narray_exPivot2': narray_exPivot2,
		'_x_plot': x_plot,
		'_y_plot': y_plot,
		'_x_plot3': x_plot3,
		'_y_plot3': y_plot3,
...
#						row_heights=[0.6, 0.2, 0.2],
						...
						x_title='timeperiod buy') #,
#						y_title='timeperiod sell')
	...
	for k in _dict.keys():
		...
		narray_exPivot2 = _dict[k]['_narray_exPivot2']
	...
	fig.update_layout(
		...
		coloraxis=dict(
			...
			colorscale='RdBu', cmin=-15, cmid=0, cmax=15
		),
		coloraxis2=dict(
			colorbar={'lenmode': 'fraction', 'len': 0.6, 'y':.2, 'tickmode': 'array', 'tickvals': [0, 1, 2, 3, 4]},
			colorscale='Greys', cmin=0, cmax=4
		),
		showlegend=False,
		autosize=False,
		width=_width,
